...
const Profile = () => {
  ...
  setProfilePic(res.data.profilePic || null); // ✅ Load the image from backend
}).catch((err) => {
  console.error(err);
 ...
 const handleProfilePicChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  try {
    const response = await api.post("http://localhost:3000/api/user/upload-profile-pic", formData, {
      headers: {
    ...
    setProfilePic(response.data.profilePic); // update new image
