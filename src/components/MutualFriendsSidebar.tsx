...
const MutualFriendsSidebar = ({ userId }: Props) => {
  ...
  useEffect(() => {
    api
      ...
      .catch((err) => console.error("Mutual fetch failed:", err));
  }, [userId]);
  ...
  return (
    <aside className="hidden lg:block fixed right-1 bottom-65 w-69 z-50">
      <div className="bg-gradient-to-br from-purple-100 to-blue-100 p-5 rounded-2xl shadow-2xl border border-gray-300">
        ...
        <ul className="space-y-4">
          {mutuals.map((mf) => (
            <li
              key={mf._id}
              className="flex items-center justify-between p-2 bg-white rounded-xl shadow-sm hover:shadow-md transition"
            >
